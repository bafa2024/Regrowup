...
if (isset($_POST['post_job'])) {
    ...
    $destination = "";
        ...
        if ($job->store($user_id, $job_title, $company, $industry, $subindustry, $location, $contract_type, $salary, $person_specifications, $skill_requirements, $qualifications_requirements, $experience_requirements, $job_description, $job_file)) {
            $job->alert_redirect("Job posted successfully", "/apps/work/ui/views/browse/browse_jobs.php");
        } else {
            
            $job->alert_redirect("Job post failed", "/apps/work/ui/views/jobs/post_job.php");
        }
...
if (isset($_POST['apply_job'])) {
    ...
    if($job->check_if_applied($job_id, $user_id)){
        ...
        if ($job->apply_job()) {
            $job->alert_redirect("Job application submitted successfully", "/apps/work/ui/views/browse/browse_jobs.php");
        } else {
            
            $job->alert_redirect("Job application failed", "/apps/work/ui/views/jobs/apply_job.php");
        }
    }
...
if (isset($_POST['delete_job'])) {
    ...
    if ($job->delete_job($job_id)) {
        ...
        $job->alert_redirect("Job  deleted successfully", "/apps/work/ui/views/manage/manage_jobs.php");

    } else {
        
        $job->alert_redirect("Job deletion failed", "/apps/work/ui/views/manage/manage_jobs.php");
...
if (isset($_POST['update_job'])) {
    ...
    if ($job->update_job()) {
        ...
        $job->alert_redirect("Job  Updated successfully", "/apps/work/ui/views/manage/manage_jobs.php");

    } else {
        
        $job->alert_redirect("Failed to Update", "/apps/work/ui/views/manage/manage_jobs.php");
...
if (isset($_POST['close_job'])) {
    ...
    if ($job->job_status($job_id, $status)) {
        ...
        $job->alert_redirect("Job  Closed successfully", "/apps/work/ui/views/manage/manage_jobs.php");

    } else {
        
        $job->alert_redirect("Failed to Close", "/apps/work/ui/views/manage/manage_jobs.php");
...
if (isset($_POST['pause_job'])) {
    ...
    if ($job->job_status($job_id, $status)) {
        ...
        $job->alert_redirect("Job  Paused successfully", "/apps/work/ui/views/manage/manage_jobs.php");

    } else {
        
        $job->alert_redirect("Failed to Paused", "/apps/work/ui/views/manage/manage_jobs.php");
