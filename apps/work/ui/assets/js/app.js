...
save.addEventListener("click", async () => {
    try {
        ...
        const res = await fetch("/api/actionAPI.php?act=create", {
            method: "POST",
            body: JSON.stringify({ 
                "name": name, 
                "age": age, 
...
const getStudents = async () => {
    try {
        ...
        let tr = "";
        const res = await fetch("/api/actionAPI.php?act=list", {
            method: "GET",
            headers: {
                "Content-Type": "application/json"
            }
        ...
        }else if(output.empty === "error"){
            tr = "<tr>Something Went Wrong</td>"
        } else {
            for (var i in output) {
                tr += `
            <tr>
...
const getStudent = async(search) => {
    //let search= "Mark";
    try {
        const tbody = document.querySelector("#tbody_s");
        let tr = "";
        const res = await fetch("/api/actionAPI.php?act=view&std="+search
...
const editStudent = async (id) => {
    ...
    const res = await fetch(`/api/actionAPI.php?act=edit&id=${id}`, {
        method: "GET",
...
update.addEventListener("click", async () => {
    ...
    const res = await fetch("/api/actionAPI.php?act=update", {
        method: "POST",
        body: JSON.stringify({
            "id": id,
            "name": name,
...
const deleteStudent = async (id) => {
    const res = await fetch("/api/actionAPI.php?act=delete&id=" + id, {
        method: "GET",
    });
    const output = await res.json();
    if (output.success) {
...
const getTotalCount = async () => {
    ...
    const res = await fetch("/api/actionAPI.php?act=count", {
        method: "GET"
    })
    const output = await res.json();
...
function logout() {
    ...
    fetch("/api/logsAPI.php?act=logout")
    .then((res) => { 
        return res.text(); 
    })
        .then((txt) => {
