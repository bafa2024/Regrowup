...
The code snippet above responds to the URLs `/blog`, `/lab/year`, `/lab/year/month`, `/lab/year/month/day`, and `/lab/year/month/day/slug`.
...
The code snipped above unfortunately also responds to URLs like `/lab/foo` and states that the overview needs to be shown - which is incorrect. Optional subpatterns can be made successive by extending the parenthesized subpatterns so that they contain the other optional subpatterns: The pattern should resemble `/blog(/year(/month(/day(/slug))))` instead of the previous `/blog(/year)(/month)(/day)(/slug)`:
