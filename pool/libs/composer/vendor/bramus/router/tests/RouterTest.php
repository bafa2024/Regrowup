...
namespace {
    ...
    class RouterTest extends PHPUnit_Framework_TestCase
    {
        ...
        public function testDynamicRouteWithOptionalNestedSubpatterns()
        {
            ...
            // Test the /lab/year route
            ob_clean();
            $_SERVER['REQUEST_URI'] = '/lab/1983';
            $router->run();
            $this->assertEquals('Blog year overview (1983)', ob_get_contents());

            // Test the /lab/year/month route
            ob_clean();
            $_SERVER['REQUEST_URI'] = '/lab/1983/12';
            $router->run();
            $this->assertEquals('Blog month overview (1983-12)', ob_get_contents());

            // Test the /lab/year/month/day route
            ob_clean();
            $_SERVER['REQUEST_URI'] = '/lab/1983/12/26';
            $router->run();
            $this->assertEquals('Blog day overview (1983-12-26)', ob_get_contents());

            // Test the /lab/year/month/day/slug route
            ob_clean();
            $_SERVER['REQUEST_URI'] = '/lab/1983/12/26/bramus';
            $router->run();
